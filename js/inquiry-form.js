...
(() => {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    // Visual loading state & YOU Loader Overlay
    let overlay = document.getElementById('inquiry-success-loading-modal');
    if (!overlay) {
      overlay = document.createElement('div');
      overlay.id = 'inquiry-success-loading-modal';
      overlay.className = 'fixed inset-0 z-[9999999] flex flex-col items-center justify-center bg-[#070a12]/92 backdrop-blur-2xl transition-all duration-400';
      overlay.innerHTML = `
        <div class="flex flex-col items-center justify-center p-8 rounded-3xl text-center max-w-sm w-full mx-4">
            <div class="loader-you flex items-center justify-center mb-5 scale-110 sm:scale-125">
            ...
            <div class="space-y-1.5 text-center">
                <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/20 border border-indigo-500/30 text-indigo-300 text-xs font-mono font-bold tracking-widest uppercase">
                    <span class="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
                    <span>CONFIRMING INQUIRY</span>
                </div>
                <p class="text-white font-extrabold text-lg sm:text-xl" style="font-family:'Outfit', sans-serif;">Preparing Your Boarding Pass...</p>
                <p class="text-white/40 text-xs font-medium">Securing cloud intake & lead routing</p>
            </div>
        </div>
      `;
